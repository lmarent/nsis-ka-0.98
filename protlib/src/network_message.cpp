...
/// ----------------------------------------------------------
/// $Id: network_message.cpp 5715 2010-10-13 19:50:44Z bless $
/// $HeadURL: https://svn.ipv6.tm.uka.de/nsis/protlib/branches/20091014-autotools/src/network_message.cpp $
// ===========================================================
//                      
 ...
 * @param s buffer size.
 */
NetMsg::NetMsg(uint32 s) : buf(NULL), buf_len(0), pos(NULL), buf_end(NULL), allocated(true) {
 ...
 */
NetMsg::NetMsg(uchar *b, uint32 s, bool copy) : buf(NULL), buf_len(0), pos(NULL), buf_end(NULL), allocated(false) {
  if (s>max_size) throw NetMsgError(NetMsgError::ERROR_TOO_LONG);
  if (s==0) throw NetMsgError(NetMsgError::ERROR_INVALID_BUFSIZE);
  if (copy) {
    buf = new(nothrow) uchar[s];
