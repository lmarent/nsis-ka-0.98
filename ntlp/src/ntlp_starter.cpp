...
/// ----------------------------------------------------------
/// $Id: ntlp_starter.cpp 5731 2010-10-23 19:59:39Z bless $
/// $HeadURL: https://svn.ipv6.tm.uka.de/nsis/ntlp/branches/20091014-autotools/src/ntlp_starter.cpp $
// ===========================================================
//                      
...
NTLPStarter::main_loop(uint32 n) 
{
  ...
  cout << color[blue] << gist_name << " v" << gist_releasestring 
       << " ($Rev: 5731 $ - development build) (C) 2005-2010 Karlsruhe Institute of Technology" << color[off] << endl;
  cout << endl;
  ...
  */
  ...
  if (param.fi_service == NULL) {
    DLog(param.name, "No Flowinfoservice provided in ntlp parameter!");
    abort();
  }
#endif  
