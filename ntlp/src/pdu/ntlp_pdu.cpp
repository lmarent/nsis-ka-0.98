...
/// NTLP/GIST PDU
/// ----------------------------------------------------------
/// $Id: ntlp_pdu.cpp 5716 2010-10-13 20:15:05Z bless $
/// $HeadURL: https://svn.ipv6.tm.uka.de/nsis/ntlp/branches/20091014-autotools/src/pdu/ntlp_pdu.cpp $
// ===========================================================
//                      
...
ntlp_pdu* ntlp_pdu::deserialize(NetMsg& msg, coding_t coding, IEErrorList& errorlist, uint32& bread, bool skip) {
	...
	if (msg.get_bytes_left()<common_header_length) {
...
void ntlp_pdu::serialize(NetMsg& msg, coding_t coding, uint32& wbytes) const {
  uint32 objwbytes = 0;
  uint32 tmpwbytes = 0;
  uint32 len = 0;
  uint32 pdulen = 0;
  ...
  // check if len is devidable by 4
  if (len%4) {
